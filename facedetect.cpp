...
std::vector<FaceInfo> CFaceDetetor::NonMaximumSuppression(std::vector<FaceInfo>& bboxes,
...
void CFaceDetetor::Bbox2Square(std::vector<FaceInfo>& bboxes){
...
std::vector<FaceInfo> CFaceDetetor::BoxRegress(std::vector<FaceInfo>& faceInfo, int stage){
...
void CFaceDetetor::Padding(int img_w, int img_h){
...
void CFaceDetetor::GenerateBoundingBox(Blob<float>* confidence, Blob<float>* reg,
...
CFaceDetetor::CFaceDetetor(){}
int CFaceDetetor::Init(const std::string &proto_model_dir, int cpuonly)
{
	m_cpuonly = cpuonly;
	threshold[0] = 0.6;
	threshold[1] = 0.7;
...
void CFaceDetetor::WrapInputLayer(std::vector<cv::Mat>* input_channels,
...
void CFaceDetetor::ClassifyFace(const std::vector<FaceInfo>& regressed_rects, cv::Mat &sample_single,
...
void CFaceDetetor::ClassifyFace_MulImage(const std::vector<FaceInfo>& regressed_rects, cv::Mat &sample_single,
	boost::shared_ptr<Net<float> >& net, double thresh, char netName){
	condidate_rects_.clear();
...
bool CFaceDetetor::CvMatToDatumSignalChannel(const cv::Mat& cv_mat, Datum* datum){
	if (cv_mat.empty())
		return false;
...
void CFaceDetetor::Detect(const cv::Mat& image, std::vector<FaceInfo>& faceInfo){
